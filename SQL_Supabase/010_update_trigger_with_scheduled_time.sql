-- ============================================
-- Migration 010: Update Trigger with scheduled_time
-- Date: 2026-02-21
-- Purpose: Trigger'da bildirim oluştururken scheduled_time ekle
--          Yevmiye talebi bildirimi 1 dakika sonra gelsin
-- ============================================

-- Drop existing trigger
DROP TRIGGER IF EXISTS trigger_auto_approve_attendance ON attendance_requests;

-- Recreate function WITH scheduled_time
...
BEGIN
  ...
  ELSE
    ...
    -- ZAMANLANMIŞ BİLDİRİM: 1 dakika sonra
    INSERT INTO notifications (
      ...
      NEW.id,
      CURRENT_TIMESTAMP + INTERVAL '1 minute'  -- 1 dakika sonra
    );
  END IF;
...
-- 
-- DEĞİŞİKLİKLER:
-- 1. scheduled_time kolonu eklendi
-- 2. Otomatik onay bildirimi: scheduled_time = NULL (anında)
-- 3. Yevmiye talebi bildirimi: scheduled_time = NOW() + 1 dakika
--
-- ETKİ:
-- - Güvenilir çalışanlar: Anında bildirim alır
-- - Güvenilir olmayan çalışanlar: Yöneticiye 1 dakika sonra bildirim gider
-- - Flutter tarafında periyodik kontrol ile bildirim gösterilir
--
