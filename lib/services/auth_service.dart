...
class AuthService {
  ...
  Future<Map<String, dynamic>?> get currentUser async {
    try {
      ...
      try {
        ...
        logError('Kullanıcı bilgileri alınırken hata', e, stack);
        userDataNotifier.value = null;
        return null;
      }
    } catch (e, stack) {
      logError('currentUser erişiminde hata', e, stack);
      return null;
    }
  }
  ...
  Future<String?> register(
    ...
    try {
      ...
      print('Kayıt işlemi başlatıldı: $lowercaseUsername');

      final usernameError = _validateUsername(lowercaseUsername);
      if (usernameError != null) {
        print('Kullanıcı adı doğrulama hatası: $usernameError');
      ...
      if (usernameAvailability != null) {
        print('Kullanıcı adı kullanılabilirlik hatası: $usernameAvailability');
        return usernameAvailability;
      }

      print('Kullanıcı kaydı yapılıyor: $lowercaseUsername');
      final response =
          await supabase
              .from('users')
              .insert({
      ...
      print('Kullanıcı kaydı başarılı, ID: $id');
      
      return null;
    } catch (e, stack) {
      logError('Kayıt sırasında hata', e, stack);
      ...
      if (e is PostgrestException) {
        ...
        if (e.code == 'P0001') {
          ...
          return 'Bu kullanıcı adı ile kayıtlı bir kullanıcı var.';
      ...
      return 'Kayıt sırasında bir hata oluştu: ${e.toString()}';
    }
  }
  ...
  Future<String?> signIn(String username, String password) async {
    try {
      final lowercaseUsername = username.toLowerCase();
      print('Giriş işlemi başlatıldı: $lowercaseUsername');
      ...
      if (result == null) {
        print('Kullanıcı bulunamadı veya şifre hatalı: $lowercaseUsername');
        userDataNotifier.value = null;
      ...
      if (isBlocked) {
        print('Kullanıcı bloklu: $lowercaseUsername');
        userDataNotifier.value = null;
      ...
      // Önceki tüm attendance kayıtlarını temizle
      try {
        // Temizlik yapmak için SharedPreferences'ı kullanıyoruz
        final prefs = await SharedPreferences.getInstance();
        
        // Eski formattaki anahtarı temizle
        await prefs.remove('attendance_date');
        
        // Bugünün tarihiyle ilgili kayıtları temizleme (önceki kullanıcıya ait olabilir)
        final today = DateTime.now();
        final todayKey = 'attendance_done_today_${today.year}_${today.month}_${today.day}';
        await prefs.remove(todayKey);
        
        print('Önceki kullanıcıya ait yevmiye kayıtları temizlendi');
      } catch (e) {
        print('Yevmiye kayıtları temizlenirken hata: $e');
      }
      ...
      logError('Giriş sırasında hata', e, stack);

      if (e is SocketException || e is ClientException) {
        // Ağ bağlantı hatası (internet yok, sunucuya erişilemiyor vb.)
        print('Ağ bağlantı hatası: ${e.toString()}');
        userDataNotifier.value = null;
  ...
  Future<bool> signOut({bool keepSavedAccounts = true}) async {
    try {
      ...
      for (int i = 0; i < savedAccountsJson.length; i++) {
        try {
          final accountData = jsonDecode(savedAccountsJson[i]) as Map<String, dynamic>;
          final userId = accountData['user_id'] as int? ?? accountData['id'] as int?;
      ...
      print('Mevcut kullanıcı hesap listesinden çıkarıldı. Kalan hesap sayısı: ${updatedAccounts.length}');
      ...
      if (updatedAccounts.isNotEmpty) {
        ...
        if (currentAccountIndex >= updatedAccounts.length) {
          ...
          nextAccountIndex = currentAccountIndex; // Aynı indeksteki hesaba geç (şimdi farklı bir hesap)
        }
        ...
        try {
          ...
          final nextAccountData = jsonDecode(nextAccountJson) as Map<String, dynamic>;
          final nextUserId = nextAccountData['user_id'] as int? ?? nextAccountData['id'] as int?;
          final nextUsername = nextAccountData['username'] as String;
          
          print('Çıkış sonrası geçilecek hesap: $nextUsername (ID: $nextUserId)');
      ...
      if (updatedAccounts.isNotEmpty && nextAccountIndex >= 0 && nextAccountIndex < updatedAccounts.length) {
        try {
          // Bir sonraki hesabın bilgilerini al
          final nextAccountJson = updatedAccounts[nextAccountIndex];
          final nextAccountData = jsonDecode(nextAccountJson) as Map<String, dynamic>;
          ...
          print('Çıkış sonrası sonraki hesaba otomatik geçiş yapılıyor: $nextUsername (ID: $nextUserId)');
            ...
            if (success) {
              ...
              Future.delayed(Duration(milliseconds: 300), () {
                try {
                  ...
                  accountsUpdateNotifier.value = !accountsUpdateNotifier.value;
                  print('Hesaplar listesi yenileme bildirimi gönderildi');
                } catch (e) {
                  print('Hesaplar listesi yenileme bildirimi gönderilirken hata: $e');
                }
              });
            }
          });
  ...
  bool _containsTurkishCharacters(String text) {
    final turkishChars = 'çğıöşüÇĞİÖŞÜ';
    return text.split('').any((char) => turkishChars.contains(char));
  }
  ...
  Future<bool> isAdmin() async {
    try {
      ...
      print('isAdmin: is_admin değeri: $isAdminValue (${isAdminValue.runtimeType})');
  ...
  Future<bool> saveCurrentUserToSavedAccounts() async {
    try {
      ...
      await prefs.setStringList('saved_accounts', updatedAccounts);
      print('Kullanıcı kaydedilmiş hesaplara eklendi/güncellendi: ${userData['username']}');
  ...
  Future<bool> autoLoginWithFirstSavedAccount() async {
    try {
      ...
      try {
        ...
        final accountData = jsonDecode(savedAccountsJson.first) as Map<String, dynamic>;
        
        // Gerekli alanlar mevcut mu kontrol et
        if (!accountData.containsKey('user_id') || !accountData.containsKey('username')) {
        ...
        print('İlk kaydedilmiş hesaba otomatik giriş yapılıyor: $username (ID: $userId)');
  ...
  Future<void> _clearSessionData() async {
    try {
      ...
      final today = DateTime.now();
      final todayKey = 'notification_sent_${today.year}_${today.month}_${today.day}';
      await prefs.remove(todayKey);
    ...
    try {
      ...
      try {
        ...
        if (currentUser != null) {
          ...
          final String currentUsername = (currentUser['username'] as String).toLowerCase();
        ...
        if (targetData != null) {
          ...
          final String targetUsername = (targetData['username'] as String).toLowerCase();
      ...
      print('Mevcut kullanıcı admin mi: $isCurrentAdmin, Hedef kullanıcı admin mi: $isTargetAdmin');
      ...
      print(isAdmin ? 'Admin hesabına geçiş hazırlanıyor...' : 'Normal kullanıcı hesabına geçiş hazırlanıyor...');
