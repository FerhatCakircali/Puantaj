...
class _NotificationSettingsScreenState extends State<NotificationSettingsScreen> with SingleTickerProviderStateMixin {
  ...
  void _filterWorkers(String query) {
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredWorkers = _workers
            .where((worker) => worker.fullName.toLowerCase().contains(query.toLowerCase()))
            .toList();
      }
    });
  }
  ...
  Future<void> _loadSettings() async {
    ...
    try {
      ...
      if (settings != null) {
        ...
        print('Ayarlar yüklendi: id=${settings.id}, enabled=${settings.enabled}, time=${settings.time}');
      } else {
        ...
        setState(() {
          ...
          _selectedTime = '08:00'; // Varsayılan saat
        });
      }
    } catch (e) {
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      ...
      final NotificationSettings settingsToSave = _settings ?? NotificationSettings(
        userId: userId,
        time: _selectedTime,
        enabled: _isEnabled,
      ...
      print('Ayarlar güncelleniyor: id=${updatedSettings.id}, enabled=$_isEnabled, time=$_selectedTime');
      
      print('updateNotificationSettings çağrılıyor...');
      final success = await _notificationService.updateNotificationSettings(updatedSettings);
      ...
      if (success) {
        ...
        final hasAttendanceToday = await _notificationService.hasAttendanceEntryForToday();
        final attendanceDoneLocally = await AttendanceCheck.isTodayAttendanceDone();
        
        if (hasAttendanceToday || attendanceDoneLocally) {
          _showSnackBar('Bildirim ayarları kaydedildi. Bugün için yevmiye girişi zaten yapılmış.');
        } else if (_isEnabled) {
          // Saat kontrolü yap
          ...
          if (scheduledTime.isBefore(now)) {
            _showSnackBar('Bildirim ayarları kaydedildi. Belirtilen saat geçtiği için bildirim yarın etkin olacak.');
          } else {
            _showSnackBar('Bildirim ayarları kaydedildi. Bildirim bugün ${_selectedTime} saatinde gönderilecek.');
          }
        } else {
          _showSnackBar('Bildirim ayarları kaydedildi. Bildirimler devre dışı bırakıldı.');
        }
  ...
  Future<void> _selectTime() async {
    ...
    if (pickedTime != null) {
      setState(() {
        _selectedTime = '${pickedTime.hour.toString().padLeft(2, '0')}:${pickedTime.minute.toString().padLeft(2, '0')}';
      });
    }
  }
              ...
              onPressed: isSubmitting ? null : () async {
                ...
                try {
                  ...
                  if (result != null) {
                    ...
                    _showSnackBar('${worker.fullName} için hatırlatıcı eklendi');
                    // Hatırlatıcı listesini güncelle
                    _loadReminders();
                  } else {
                    _showSnackBar('Hatırlatıcı eklenirken bir hata oluştu');
                  }
                } catch (e) {
                  print('Hatırlatıcı eklenirken hata: $e');
                  _showSnackBar('Hatırlatıcı eklenirken bir hata oluştu: $e');
                } finally {
                  setDialogState(() {
                    isSubmitting = false;
                  });
                              ...
                              children: [
                                ...
                                SwitchListTile(
                                  ...
                                  onChanged: (value) {
                                    ...
                                    print('Hatırlatıcı durumu değiştirildi: $_isEnabled');
                                  },
                                  secondary: Icon(
                                    _isEnabled ? Icons.notifications_active : Icons.notifications_off,
                                    color: _isEnabled ? Theme.of(context).colorScheme.primary : null,
                                  ),
                                ),
                                const Divider(),
                                ListTile(
                        ...
                        Row(
                          ...
                          children: [
                            Expanded(
                              child: CommonButton(
                                onPressed: _isLoading ? () {} : () {
                                  print('Ayarları Kaydet butonuna tıklandı');
                                  _saveSettings();
                                },
                            ...
                            Expanded(
                              child: CommonButton(
                                onPressed: _isLoading ? () {} : () {
                                  print('Test Bildirimi Gönder butonuna tıklandı');
                                      ...
                                      : ListView.builder(
                                          ...
                                          itemBuilder: (context, index) {
                                            ...
                                            return ListTile(
                                              ...
                                              onTap: () => _showEmployeeReminderDialog(worker),
                                            );
                                          },
                                        ),
                            ],
          ...
          _isLoadingReminders
              ...
              : Expanded(
                  child: ListView.builder(
                    itemCount: _reminders.length + 1, // +1 for "Yeni Hatırlatıcı Ekle" section
                    itemBuilder: (context, index) {
                      ...
                      final isToday = reminder.reminderDate.day == DateTime.now().day &&
