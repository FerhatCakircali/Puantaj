...
final GoRouter _appRouter = GoRouter(
  ...
  routes: [
    ...
    GoRoute(path: '/admin_accounts', builder: (context, _) => const AdminPanelScreen(initialTabIndex: 1)),
  ],
);
...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _blockCheckTimer = Timer.periodic(const Duration(seconds: 60), (_) => _checkUserBlockStatus());
  }
  ...
  Future<void> _checkUserBlockStatus() async {
    try {
      ...
      if (isBlocked && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Bu hesap yönetici tarafından engellenmiştir. Lütfen iletişime geçin: ferhatcakircali@gmail.com'),
