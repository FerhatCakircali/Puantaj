...
class EmployeeReminderDetailScreen extends StatefulWidget {
  ...
  const EmployeeReminderDetailScreen({
    Key? key,
    this.reminderId,
  }) : super(key: key);

  @override
  State<EmployeeReminderDetailScreen> createState() => _EmployeeReminderDetailScreenState();
}

class _EmployeeReminderDetailScreenState extends State<EmployeeReminderDetailScreen> {
  ...
  Future<void> _loadReminder() async {
    ...
    try {
      ...
      final reminders = await _reminderService.getEmployeeReminders(includeCompleted: true);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Çalışan Hatırlatıcısı'),
      ),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('Bu hatırlatıcı bildirim mesajı silinecek. Onaylıyor musunuz?'),
          actions: [
            TextButton(
              onPressed: () {
  ...
  Future<void> _deleteReminder() async {
    ...
    try {
      // Hatırlatıcıyı sil
      final success = await _reminderService.deleteEmployeeReminder(_reminder!.id!);
      
      if (success) {
        _showSnackBar('Hatırlatıcı başarıyla silindi');
        
        // Bildirim ayarları sayfasına yönlendirme yapmak yerine 
        // Ana sayfaya yönlendir ve menünün görünmesini sağla
        context.go('/home');
        
        // Kısa bir gecikme sonra drawer'ı aç
  ...
  Widget _buildDrawer(BuildContext context) {
    ...
    return Drawer(
      width: MediaQuery.of(context).size.width * 0.75, // Çekmece genişliği ekranın %75'i
          ...
          Padding(
            ...
            child: Material(
              ...
              child: InkWell(
                ...
                splashColor: Theme.of(context).colorScheme.error.withOpacity(0.15),
                child: Padding(
                  padding: const EdgeInsets.symmetric(
                    horizontal: 16.0,
                    vertical: 12.0,
