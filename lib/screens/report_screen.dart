...
class _ReportScreenState extends State<ReportScreen>
  ...
  Future<void> _loadEmployees() async {
    ...
    try {
      ...
      print('Çalışanları yükleme hatası: $e');
      if (mounted) {
  ...
  Future<void> _loadData() async {
    ...
    final activeEmployees =
        allEmployees.where((emp) => activeWorkerIds.contains(emp.id)).toList();
  ...
  Future<Map<String, dynamic>> _calculateAttendanceStats(
    ...
    DateTime currentDate =
        emp.startDate.isAfter(_startDate) ? emp.startDate : _startDate;
  ...
  Future<void> _createPeriodReport() async {
    ...
    try {
      ...
      final robotoFontBytes =
          (await rootBundle.load(
            'assets/fonts/Roboto-Regular.ttf',
          )).buffer.asUint8List();
      final robotoBoldFontBytes =
          (await rootBundle.load(
              ...
              ? await (() async {
                ...
                final file = await _pdfService
                    .generatePeriodEmployeeReportWithProgress(
                      ...
                      attendances: await _attendanceService
                      ...
                      progressCallback:
                          (progress) => _progressNotifier.value = progress,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Row(
              children: [
                Expanded(child: Text('Dönemsel rapor oluşturuldu')),
                IconButton(
                  icon: const Icon(Icons.share, color: Colors.white),
                  tooltip: 'Paylaş',
                  onPressed:
  ...
  Future<void> _selectCustomDate(bool isStartDate) async {
    final DateTime initialDate =
        isStartDate ? _customStartDate : _customEndDate;
  ...
  static Future<String> _generatePeriodGeneralReportInIsolate(
    ...
    final employees =
        (args['employees'] as List)
            .map((e) => Employee.fromMap(Map<String, dynamic>.from(e)))
            .toList();
    final allAttendances =
        (args['allAttendances'] as List)
            .map(
              (list) =>
                  (list as List)
                      .map(
                        (a) => attendance.Attendance.fromMap(
